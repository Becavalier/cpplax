...
// Do semantic analysis based on the ASTs generated by parser.
struct Resolver : public ExprVisitor, public StmtVisitor {
  ...
  void resolveLocal(Expr::sharedConstExprPtr expr, const Token& name) {
    ...
    for (auto i = scopes.size(); i >= 1; i--) {
      if (scopes.at(i - 1).contains(name.lexeme)) {
        interpreter->resolve(expr, scopes.size() - 2 - i);
      }
    }
  }
  void resolveFunction(std::shared_ptr<const FunctionStmt> function) {
    beginScope();
  ...
  typeRuntimeValue visitVariableExpr(std::shared_ptr<const VariableExpr> expr) override { 
    ...
    if (!scopes.empty()) {
      ...
      if (exist != scopes.back().end() && !exist->second) Error::error(expr->name, "can't read local variable in its own initializer.");
    }
